...
export default class Manifest extends EventEmitter<IManifestEvents> {
  ...
  private _timeBounds : {
     ...
     *
     * Together with `windowDepth` and the `maximumTimeData` object, this value
     * allows to compute at any time the minimum seekable time:
     *
     *   - if `windowDepth` is not set, the minimum seekable time is a constant
     *     that corresponds to this value.
     *
     *    - if `windowDepth` is set, `absoluteMinimum` will act as the absolute
     *      minimum seekable time we can never seek below, even when
     *      `windowDepth` indicates a possible lower position.
     ...
     *      segment, and `windowDepth` would be the whole depth that will become
     *      available once enough segments have been generated.
     */
  ...
  public getMinimumPosition() : number {
    ...
    if (windowData.windowDepth === null) {
    ...
    const theoricalMinimum = maximumTime - windowData.windowDepth;
